...
$h = mysqli_query($connect, "select harga_per_kilo from harga");
...
mysqli_query($connect, "update transaksi set transaksi_pelanggan='$pelanggan', transaksi_harga='$harga', transaksi_berat='$berat', transaksi_tgl_selesai='$tgl_selesai', transaksi_status='$status' where transaksi_id='$id'");
...
for ($x = 0; $x < count($jenis_pakaian); $x++) {
    if ($jenis_pakaian[$x] != "") {
        mysqli_query($connect, "insert into pakaian values('', '$id', '$jenis_pakaian[$x]', '$jumlah_pakaian[$x]')");
