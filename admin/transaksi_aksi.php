...
mysqli_query($connect, "INSERT into transaksi VALUES('', '$tgl_hari_ini', '$pelanggan', '$harga', '$berat', '$tgl_selesai', '$status')");

// menyimpan id dari data yang di simpan pada query insert data sebelumnya
$id_terakhir = mysqli_insert_id($connect);
...
for ($x=0;$x<count($jenis_pakaian);$x++) {
    if ($jenis_pakaian[$x] != "") {
        mysqli_query($connect, "INSERT into pakaian VALUES('', '$id_terakhir', '$jenis_pakaian[$x]', '$jumlah_pakaian[$x]')");
